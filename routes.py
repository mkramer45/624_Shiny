...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///C:\\Users\\Mike\\Desktop\\PythonSuccess\\20190619_PythonAnywhereMirror22\\SurfSend.db'
...
class RegisterForm(FlaskForm):
	...
	username = StringField('Desired Username', validators=[InputRequired(), Length(min=1, max=15)])
	firstname = StringField('Your First Name', validators=[InputRequired(), Length(min=1, max=15)])
	cellphone = StringField('Cell Phone #', validators=[InputRequired(), Length(min=4, max=15)])
	homeaddress = StringField('Home Address', validators=[InputRequired(), Length(min=4, max=25)])
	licenseplate = StringField('License Plate #', validators=[InputRequired(), Length(min=4, max=15)])
...
class User(UserMixin, db.Model):
	...
	homeaddress = db.Column(db.String(80))
	licenseplate = db.Column(db.String(80))
...
def scrapelist2():
	if request.method == 'POST':
		...
		cursor.execute("INSERT INTO Appt VALUES (NULL,?,?,?)", (usrn,Date,Time))
		conn.commit()
		cursor.close()
		conn.close()
	...
	usrn3 = f'"{usrn2}"'
	g.db = connect_db()
	cur = g.db.execute("select ID,ApptDate, ApptTime from Appt where Username = (?)", (usrn2,))
	cur3 = g.db.execute("select Firstname from user where Username in (select Username from Appt where Username = (?))", (usrn2,))
	# cur2 = g.db.execute('select * from Tracks where artist in (select DJname from ArtistMonitor)')
	pull = [dict(ID=row[0], Date=row[1], Time=row[2]) for row in cur.fetchall()]
	pull3 = [dict(FirstName3=row[0]) for row in cur3.fetchall()]
	g.db.close()
	return render_template('scrapelist2.html', selected='submit', pull=pull,usrn2=usrn2, usrn3=usrn3, pull3=pull3)


# msk621
@app.route('/delete_appt/<ID>', methods=['POST'])
@csrf.exempt
def delete_appt(ID):
	conn = sqlite3.connect('SurfSend.db')
	cursor = conn.cursor()
	usrn2 = current_user.username
	cur = cursor.execute("select ID from Appt where Username = (?)", (usrn2,))
	del1 = [dict(ID=row[0]) for row in cur.fetchall()]
#	Date1 = request.args.get('Date')
	cursor.execute("DELETE FROM Appt WHERE ID = ?", (ID,))
	conn.commit()
	cursor.close()
	conn.close()
...
def register():
	...
	if form.validate_on_submit():
		hashed_password = bcrypt.generate_password_hash(form.password.data).decode('utf-8')
		new_user = User(firstname=form.firstname.data,username=form.username.data, email=form.email.data, cellphone=form.cellphone.data, homeaddress=form.homeaddress.data, licenseplate=form.licenseplate.data, password=hashed_password)
